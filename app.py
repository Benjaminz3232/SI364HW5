...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://postgres:asdfhero@localhost/benjaminHW5"
## Provided:
...
class UpdateToDoItemForm(FlaskForm):
    new_priority = IntegerField("new priority for this item:", validators=[Required()])
...
def delete(lst):
    ...
    db.session.delete(l)
    db.session.commit()
    flash("Successfully deleted list: {}".format(l_title))
    return redirect(url_for('all_lists')
